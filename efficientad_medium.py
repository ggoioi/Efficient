...
#명령줄 인수 구문 분석
#해당 함수를 통해 스크립트를 실행할 때 데이터셋, 하위 데이터셋, 모델 크기 및 다양한 파일 경로와 같은 다양한 매개변수를 지정할 수 있음.
def get_argparse():
    parser = argparse.ArgumentParser()
    parser.add_argument('-d','--dataset', default = 'mvtec_ad')
    parser.add_argument('-b', '--data_dir', default='./Data2',
                        help='Root directory for your dataset')
    parser.add_argument('-o', '--output_dir', default='./output/2',
                        help='Directory for saving output/results')
    ...
    parser.add_argument('-w', '--weights', default='./models/teacher_medium.pth',
                        help='Path to the weights of the pretrained teacher model')
...
#
#     valid_labels = [f.split("\\")[small_v1] for f in valid_images]
#     test_labels = [f.split("\\")[small_v1] for f in test_images]
#     train_labels = [f.split("\\")[small_v1] for f in train_images]
#     return train_images, valid_images, test_images
# # main 함수 훈련을 진행하는 main
def main():
    ...
    # else:
    #     raise Exception('Unknown config.dataset')
    # train_set, validation_set, test_set = ImagePreprocessing(dataset_path)
    ...
    torch.save(teacher, os.path.join(train_output_dir, 'teacher_medium_final.pth'))
    torch.save(student, os.path.join(train_output_dir, 'student_medium_final.pth'))
    torch.save(autoencoder, os.path.join(train_output_dir,
                                         'autoencoder_medium_final.pth'))
