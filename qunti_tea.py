...
for layers_to_fuse in fusion_layers:
    torch.quantization.fuse_modules(tea, layers_to_fuse, inplace=True)
...
# 양자화 준비
tea.qconfig = my_qconfig
torch.quantization.prepare(tea, inplace=True)
...
# 양자화 적용
torch.quantization.convert(tea, inplace=True)

# 모델 저장
torch.save(tea, os.path.join(save_path, 'teacher_quantized.pth'))
